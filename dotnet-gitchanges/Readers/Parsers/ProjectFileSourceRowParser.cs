    ...
    public class ProjectFileSourceRowParser : IRowParser<ProjectChange>
    {
        ...
        public ProjectChange Parse(string line)
        {
            ...
            try
            {
                ...
                {
                    case 5:
                        change = new ProjectChange(project: values[0], version: values[1], tag: values[2], summary: values[3], date: DateTimeOffset.ParseExact(values[4], DateFormat, CultureInfo.InvariantCulture));
                        break;
                    case 6:
                        change = new ProjectChange(project: values[0], reference: values[1], version: values[2], tag: values[3], summary: values[4], date: DateTimeOffset.ParseExact(values[5], DateFormat, CultureInfo.InvariantCulture));
                        break;
                    default:
