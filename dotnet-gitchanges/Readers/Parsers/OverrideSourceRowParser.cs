    ...
    public class OverrideSourceRowParser : IRowParser<OverrideChange>
    {
        ...
        public OverrideChange Parse(string line)
        {
            ...
            try
            {
                ...
                {
                    case 5:
                        change = new OverrideChange(id: values[0], version: new ChangeVersion(values[1]), changeType: values[2], summary: values[3], date: DateTimeOffset.ParseExact(values[4], DateFormat, CultureInfo.InvariantCulture));
                        break;
                    case 6:
                        change = new OverrideChange(id: values[0], reference: values[1], version: new ChangeVersion(values[2]), changeType: values[3], summary: values[4], date: DateTimeOffset.ParseExact(values[5], DateFormat, CultureInfo.InvariantCulture));
                        break;
                    default:
